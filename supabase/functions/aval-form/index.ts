...
Deno.serve(async (req) => {
  ...
  const { data: properties, error: propError } = await admin
    .from("av_properties")
    .select(
      "id, name, color, stage, address, summary, extra_criteria, questions, floor_plan_url, photo_urls, payment_terms, position, units:av_units(id, name, table_value, photo_urls, payment_terms, position)",
    )
