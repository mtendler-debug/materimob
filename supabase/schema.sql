...
-- ---------------------------------------------------------------------
-- Painel de desempenho da equipe pra organizações — via
-- organization_members, não av_launches (imobiliária não tem
-- lançamento).
-- ---------------------------------------------------------------------
...
-- KPIs de vendas nos painéis: ticket médio, previsão (propostas com
-- intenção de compra) e vendas por corretor/imobiliária/incorporadora.
-- Toda a base já existe (av_launch_units e av_portfolio_units.status/
-- reserved_by, preenchidos por reserve_launch_unit/reserve_portfolio_unit
-- e confirmados manualmente pelo gerente/dono) — esta migração só lê
-- em cima do que já está gravado.
...
begin
  ...
  select jsonb_build_object(
    ...
    'ticket_medio_vendas', (
      select round(avg(lu.table_value)::numeric, 2)
      from av_launch_units lu join av_launches l on l.id = lu.launch_id
      where l.organization_id = p_org_id and lu.status = 'vendida'
    ),
    'previsao_vendas', (
    ...
    'por_lancamento', (
      ...
      from (
        select
          ...
          (select count(*) from av_launch_units lu where lu.launch_id = l.id and lu.status = 'reservada') as reservadas,
          (select count(*) from av_launch_units lu where lu.launch_id = l.id and lu.status = 'vendida') as vendidas,
          (select round(avg(lu.table_value)::numeric, 2) from av_launch_units lu
            where lu.launch_id = l.id and lu.status = 'vendida') as ticket_medio_vendas,
          (select count(*) from av_selections s where s.launch_id = l.id) as total_roteiros,
          (select count(distinct s.user_id) from av_selections s where s.launch_id = l.id) as total_corretores,
          (select count(*) from av_evaluations e join av_selections s on s.id = e.selection_id where s.launch_id = l.id) as total_avaliacoes,
          (select round(avg(e.overall_score)::numeric, 1) from av_evaluations e join av_selections s on s.id = e.selection_id
    ...
    'por_parceiro', (
      ...
      from (
        select
          ...
          (select count(*) from av_launch_units lu
            join av_launches l on l.id = lu.launch_id
            join organization_members om on om.user_id = lu.reserved_by and om.organization_id = o.id
            where l.organization_id = p_org_id and lu.status = 'vendida') as total_vendidas,
          (select round(avg(lu.table_value)::numeric, 2) from av_launch_units lu
            join av_launches l on l.id = lu.launch_id
            join organization_members om on om.user_id = lu.reserved_by and om.organization_id = o.id
            where l.organization_id = p_org_id and lu.status = 'vendida') as ticket_medio_vendas
        from (
          select distinct lp.organization_id
          from av_launch_partners lp
          join av_launches l on l.id = lp.launch_id
      ...
      from (
        select
          ...
          (select count(*) from av_proposals pr join av_selections s2 on s2.id = pr.selection_id
            where s2.user_id = s.user_id and pr.buy_intent
              and s2.launch_id in (select id from av_launches where organization_id = p_org_id)) as propostas_interesse,
          (select count(*) from av_launch_units lu join av_launches l2 on l2.id = lu.launch_id
            where lu.reserved_by = s.user_id and lu.status = 'vendida' and l2.organization_id = p_org_id) as total_vendidas,
          (select round(avg(lu.table_value)::numeric, 2) from av_launch_units lu join av_launches l2 on l2.id = lu.launch_id
            where lu.reserved_by = s.user_id and lu.status = 'vendida' and l2.organization_id = p_org_id) as ticket_medio_vendas
        from av_selections s
        join av_launches l on l.id = s.launch_id
        join auth.users u on u.id = s.user_id
        left join profiles p on p.id = s.user_id
...
begin
  ...
  select jsonb_build_object(
    ...
    'total_vendas', (
      select count(*) from (
        select 1 from av_launch_units
          where reserved_by in (select user_id from organization_members where organization_id = p_org_id) and status = 'vendida'
        union all
        select 1 from av_portfolio_units
          where reserved_by in (select user_id from organization_members where organization_id = p_org_id) and status = 'vendida'
      ) x
    ),
    'ticket_medio_vendas', (
      select round(avg(v)::numeric, 2) from (
        select table_value v from av_launch_units
          where reserved_by in (select user_id from organization_members where organization_id = p_org_id) and status = 'vendida'
        union all
        select table_value v from av_portfolio_units
          where reserved_by in (select user_id from organization_members where organization_id = p_org_id) and status = 'vendida'
      ) x
    ),
    'previsao_vendas', (
      select coalesce(sum(pr.value), 0)
      from av_proposals pr join av_selections s on s.id = pr.selection_id
      where s.user_id in (select user_id from organization_members where organization_id = p_org_id) and pr.buy_intent
    ),
    'por_corretor', (
      select coalesce(jsonb_agg(t order by t.total_roteiros desc), '[]'::jsonb)
      from (
        select
          ...
          (select count(*) from av_proposals pr join av_selections s on s.id = pr.selection_id
            where s.user_id = m.user_id) as total_propostas,
          (select count(*) from (
            select 1 from av_launch_units where reserved_by = m.user_id and status = 'vendida'
            union all
            select 1 from av_portfolio_units where reserved_by = m.user_id and status = 'vendida'
          ) x) as total_vendas,
          (select round(avg(v)::numeric, 2) from (
            select table_value v from av_launch_units where reserved_by = m.user_id and status = 'vendida'
            union all
            select table_value v from av_portfolio_units where reserved_by = m.user_id and status = 'vendida'
          ) x) as ticket_medio_vendas,
          exists(select 1 from av_selections s where s.user_id = m.user_id
            and s.created_at > now() - interval '30 days') as ativo_30d
        from organization_members m
        join auth.users u on u.id = m.user_id
      ...
      from (
        select
          ...
          (select count(*) from av_selections s join av_launches l on l.id = s.launch_id
            where l.organization_id = o.id
              and s.user_id in (select user_id from organization_members where organization_id = p_org_id)) as total_roteiros,
          (select count(*) from av_launch_units lu join av_launches l on l.id = lu.launch_id
            where l.organization_id = o.id and lu.status = 'vendida'
              and lu.reserved_by in (select user_id from organization_members where organization_id = p_org_id)) as total_vendas,
          (select round(avg(lu.table_value)::numeric, 2) from av_launch_units lu join av_launches l on l.id = lu.launch_id
            where l.organization_id = o.id and lu.status = 'vendida'
              and lu.reserved_by in (select user_id from organization_members where organization_id = p_org_id)) as ticket_medio_vendas
        from (
...
-- (linha 847). Nenhuma tela ou policy deve reimplementar essa lógica
-- na mão.
...
-- Bucket novo — "imoveis" (linha 1287) só aceita imagem e 8MB; book é
-- PDF e pode ser maior.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
